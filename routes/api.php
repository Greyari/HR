...
Route::get('/test-fcm', function () {
    try {
        ...
        $deviceToken = 'dol196SPQUGdgnwusfZR9K:APA91bEayKNGyev802YpZB83OhPA9Au6e1xKFljsn89yU9yrOHIRCcTYOzyJ0chBua73YAkF9DzzHF_lxFRIvXlO-cxp3dr7vMCaBrQkw55PviNq7ieNdMY'; // token HP kamu

        $message = CloudMessage::withTarget('token', $deviceToken)
            ->withNotification(Notification::create('Test Notif', 'Ini notifikasi testing'));

        $messaging->send($message);

        return 'FCM berhasil dikirim!';
...
Route::get('/test-jwt', function () {
    try {
        $credentials = new ServiceAccountCredentials(
            null,
            storage_path('firebase/fcm-hris-5763a-9d6e182663f4.json')
        );
